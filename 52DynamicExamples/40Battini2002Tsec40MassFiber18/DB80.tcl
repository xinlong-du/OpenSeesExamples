...
for {set i 1} {$i<82} {incr i 1} {
	node $i [expr -0.0625+0.0625*$i] 0.0 0.0;
}
for {set i 101} {$i<182} {incr i 1} {
	node $i [expr -0.0625+0.0625*($i-100)] -0.047368 0.0;
}
...
#set up mass
mass $StartCentroid 1.864375 1.864375 1.864375 0.0 0.0 0.0;
  mass $EndCentroid 1.864375 1.864375 1.864375 0.0 0.0 0.0;
for {set i [expr $StartCentroid+1]} {$i<$EndCentroid} {incr i 1} {
mass $i 3.72875 3.72875 3.72875 0.0 0.0 0.0;	
}

# Define RECORDERS -------------------------------------------------------------
recorder Node -file $dataDir/DB80.out -time -node $EndCentroid -dof 1 2 3 4 5 6 disp;			# displacements of end node
