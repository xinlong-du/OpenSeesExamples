...
for {set i 101} {$i<142} {incr i 1} {
	node $i [expr -0.125+0.125*($i-100)] 0.054819277108434 0.029819277108434;
}
...
#set up mass
mass $StartCentroid 2.931975 2.931975 2.931975 0.0 0.0 0.0;
  mass $EndCentroid 2.931975 2.931975 2.931975 0.0 0.0 0.0;
for {set i [expr $StartCentroid+1]} {$i<$EndCentroid} {incr i 1} {
mass $i 5.86395 5.86395 5.86395 0.0 0.0 0.0;	
}

# Define RECORDERS -------------------------------------------------------------
recorder Node -file $dataDir/DB40Fine.out -time -node $EndLoad -dof 1 2 3 4 5 6 disp;			# displacements of end node
