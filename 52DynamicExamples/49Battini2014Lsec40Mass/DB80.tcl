...
for {set i 101} {$i<182} {incr i 1} {
	node $i [expr -0.0625+0.0625*($i-100)] 0.054819277108434 0.029819277108434;
}
node 201 2.5 -0.009 -0.009;
node 202 5.0 -0.009 -0.009;
...
geomTransf $ColTransfType  $IDColTransf 0 -0.875598373888054 0.483039840639046;	#define geometric transformation: performs a corotational geometric transformation
...
set nodeJ $i;
     element elasticBeamColumn $elemID $nodeI $nodeJ $Arig $Es $Gs $Jrig $Iy $Iz $IDColTransf $y0rig $z0rig $betaY $betaZ;	
}
element elasticBeamColumn $MiddleLoad $MiddleNode $MiddleLoad $Arig $Es $Gs $Jrig $Iy $Iz $IDColTransf $y0rig $z0rig $betaY $betaZ;
         element elasticBeamColumn $EndLoad $EndNode $EndLoad $Arig $Es $Gs $Jrig $Iy $Iz $IDColTransf $y0rig $z0rig $betaY $betaZ;

#set up mass
mass $StartCentroid 1.4659875 1.4659875 1.4659875 0.0 0.0 0.0;
  mass $EndCentroid 1.4659875 1.4659875 1.4659875 0.0 0.0 0.0;
for {set i [expr $StartCentroid+1]} {$i<$EndCentroid} {incr i 1} {
mass $i 2.931975 2.931975 2.931975 0.0 0.0 0.0;	
}

# Define RECORDERS -------------------------------------------------------------
recorder Node -file $dataDir/DB80.out -time -node $EndLoad -dof 1 2 3 4 5 6 disp;			# displacements of end node
...
# create load pattern
set WindSeriesA "Series -dt 0.000625 -filePath loadHistory.txt -factor 1";	# define acceleration vector from file (dt=0.01 is associated with the input file gm)
