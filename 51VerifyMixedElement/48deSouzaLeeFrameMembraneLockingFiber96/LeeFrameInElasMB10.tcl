...
for {set i 1} {$i<7} {incr i 1} {
	node $i 0.0 [expr -240.0+240.0*$i] 0.0;
}
for {set i 7} {$i<12} {incr i 1} {
	node $i [expr 240.0*($i-6)] 1200.0 0.0;
}
...
# Define RECORDERS -------------------------------------------------------------
recorder Node -file $dataDir/mixedLeeFrameInElasDB10.out -time -node $MiddleNode -dof 1 2 3 4 5 6 disp;			# displacements of middle node
...
#integrator ArcLength 4.0 0.1; #arclength alpha
