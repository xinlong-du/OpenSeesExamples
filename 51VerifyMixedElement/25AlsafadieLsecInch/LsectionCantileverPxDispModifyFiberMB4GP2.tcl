...
for {set i 1} {$i<6} {incr i 1} {
	node $i [expr -13.7795+13.7795*$i] 0 0;
}
...
#set SecTagTorsion 70
	...
	set Es [expr 27999.0*$ksi];		# Steel Young's Modulus
	set nu 0.3;
	...
	set J [expr  0.02653037*$in4];
	set GJ [expr $Gs*$J];
	set z0 [expr 0.64615948*$in];
	set y0 [expr -0.68585161*$in];
source Lsection.tcl;
	#set oC 0.0;
...
# Define RECORDERS -------------------------------------------------------------
recorder Node -file $dataDir/mixedCantileverDispModifyFiber2layerMB4GP2.out -time -node $EndNode -dof 1 2 3 4 5 6 disp;			# displacements of middle node
recorder Node -file $dataDir/mixedCantileverReacModifyFIber2layerMB4GP2.out -time -node $StartNode -dof 1 2 3 4 5 6 reaction;		# support reaction
