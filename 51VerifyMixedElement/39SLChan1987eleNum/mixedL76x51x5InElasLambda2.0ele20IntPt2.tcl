...
for {set i 1} {$i<22} {incr i 1} {
	node $i [expr -89.8806+89.8806*$i] 0 0;
}
...
# Define RECORDERS -------------------------------------------------------------
recorder Node -file $dataDir/mixedL76x51x5InElasLambda20End20eleBCIntPt2.out -time -node $EndNode -dof 1 2 3 4 5 6 disp;			# displacements of middle node
recorder Node -file $dataDir/mixedL76x51x5InElasLambda20Sat20eleBCIntPt2.out -time -node $StartNode -dof 1 2 3 4 5 6 disp;		# support reaction
recorder Node -file $dataDir/mixedL76x51x5InElasLambda20Mid20eleBCIntPt2.out -time -node $MiddleNode -dof 1 2 3 4 5 6 disp;
# Define DISPLAY -------------------------------------------------------------
...
set Dincr 0.0004; #-0.00002
                               #Node,  dof, 1st incr, Jd, min,   max
#integrator DisplacementControl $MiddleNode 4   $Dincr     1  $Dincr 0.0001;
integrator DisplacementControl $StartNode 1   $Dincr     1  $Dincr 0.0004;
