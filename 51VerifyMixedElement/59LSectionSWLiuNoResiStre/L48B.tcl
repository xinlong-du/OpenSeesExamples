...
for {set i 1} {$i<12} {incr i 1} {
	node $i [expr -121.92+121.92*$i] 0 0;
}
...
set ColSecTag 1
	...
	set J 18513.26128128;
	set GJ [expr $Gs*$J];
	source Lsection48B.tcl;
# define ELEMENTS-----------------------------------------------------------------------------------------------
...
# Define RECORDERS -------------------------------------------------------------
recorder Node -file $dataDir/L48BEnd.out -time -node $EndNode -dof 1 2 3 4 5 6 disp;			# displacements of middle node
recorder Node -file $dataDir/L48BSat.out -time -node $StartNode -dof 1 2 3 4 5 6 disp;		# support reaction
recorder Node -file $dataDir/L48BMid.out -time -node $MiddleNode -dof 1 2 3 4 5 6 disp;
# Define DISPLAY -------------------------------------------------------------
