...
for {set i 1} {$i<12} {incr i 1} {
	node $i [expr -122.40+122.40*$i] 0 0;
}
...
set ColSecTag 1
	...
	set Es 207000.0;		# Steel Young's Modulus
	set nu 0.3;
	...
	set J 18033.70833333;
	set GJ [expr $Gs*$J];
	source Lsection48A.tcl;
# define ELEMENTS-----------------------------------------------------------------------------------------------
...
# Define RECORDERS -------------------------------------------------------------
recorder Node -file $dataDir/L48AEnd.out -time -node $EndNode -dof 1 2 3 4 5 6 disp;			# displacements of middle node
recorder Node -file $dataDir/L48ASat.out -time -node $StartNode -dof 1 2 3 4 5 6 disp;		# support reaction
recorder Node -file $dataDir/L48AMid.out -time -node $MiddleNode -dof 1 2 3 4 5 6 disp;
# Define DISPLAY -------------------------------------------------------------
...
integrator DisplacementControl $EndNode 1   $Dincr     1  $Dincr -0.0005;
