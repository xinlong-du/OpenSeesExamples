...
for {set i 1} {$i<6} {incr i 1} {
	node $i [expr -89.875+89.875*$i] 0 0;
}
...
# Define RECORDERS -------------------------------------------------------------
recorder Node -file $dataDir/L76x51x5Lambda04End4eleBC.out -time -node $EndNode -dof 1 2 3 4 5 6 disp;			# displacements of middle node
recorder Node -file $dataDir/L76x51x5Lambda04Sat4eleBC.out -time -node $StartNode -dof 1 2 3 4 5 6 disp;		# support reaction
recorder Node -file $dataDir/L76x51x5Lambda04Mid4eleBC.out -time -node $MiddleNode -dof 1 2 3 4 5 6 disp;
# Define DISPLAY -------------------------------------------------------------
