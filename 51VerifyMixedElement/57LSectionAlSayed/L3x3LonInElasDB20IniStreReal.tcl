...
for {set i 1} {$i<22} {incr i 1} {
	node $i [expr -4.540515+4.540515*$i] 0 0;
}
...
# Define RECORDERS -------------------------------------------------------------
recorder Node -file $dataDir/L3x3LonDB20IniStreEndReal.out -time -node $EndNode -dof 1 2 3 4 5 6 disp;			# displacements of middle node
recorder Node -file $dataDir/L3x3LonDB20IniStreSatReal.out -time -node $StartNode -dof 1 2 3 4 5 6 disp;		# support reaction
recorder Node -file $dataDir/L3x3LonDB20IniStreMidReal.out -time -node $MiddleNode -dof 1 2 3 4 5 6 disp;
# Define DISPLAY -------------------------------------------------------------
