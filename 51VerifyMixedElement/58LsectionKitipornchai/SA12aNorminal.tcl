...
set ColSecTag 1
	...
	set Es 214000.0;		# Steel Young's Modulus
	set nu 0.3;
	...
	set J 15699.39583333;
	set GJ [expr $Gs*$J];
	source LsectionNorminal.tcl;
# define ELEMENTS-----------------------------------------------------------------------------------------------
...
# Define RECORDERS -------------------------------------------------------------
recorder Node -file $dataDir/SA12aEndNorm.out -time -node $EndNode -dof 1 2 3 4 5 6 disp;			# displacements of middle node
recorder Node -file $dataDir/SA12aSatNorm.out -time -node $StartNode -dof 1 2 3 4 5 6 disp;		# support reaction
recorder Node -file $dataDir/SA12aMidNorm.out -time -node $MiddleNode -dof 1 2 3 4 5 6 disp;
# Define DISPLAY -------------------------------------------------------------
