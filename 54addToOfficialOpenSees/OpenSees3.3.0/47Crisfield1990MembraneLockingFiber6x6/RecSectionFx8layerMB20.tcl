...
for {set i 1} {$i<12} {incr i 1} {
	node $i [expr -24.0+24.0*$i] 0 0;
}
for {set i 12} {$i<22} {incr i 1} {
	node $i 240.0 [expr -24.0*($i-11)] 0;
}
...
set ColSecTag 1
	...
	set Es 71240.0;		# Steel Young's Modulus
	set nu 0.31;
...
# Define RECORDERS -------------------------------------------------------------
recorder Node -file $dataDir/mixedDispEnd8Layer20ele.out -time -node $EndNode -dof 1 2 3 4 5 6 disp;			# displacements of end node
recorder Node -file $dataDir/mixedDispMid8Layer20ele.out -time -node $MiddleNode -dof 1 2 3 4 5 6 disp;			# displacements of middle node
