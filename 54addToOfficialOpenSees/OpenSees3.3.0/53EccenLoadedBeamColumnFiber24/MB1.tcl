...
for {set i 1} {$i<3} {incr i 1} {
	node $i [expr -262.54+262.54*$i] 0 0;
}
node 3 262.54 0.82 0;
...
element mixedBeamColumnAsym 2 $EndNode $EndLoad $numIntgrPts $ColSecTag $IDColTransf -shearCenter $y0 $z0 -integration Lobatto;
# Define RECORDERS -------------------------------------------------------------
recorder Node -file $dataDir/MB1End.out -time -node $EndNode -dof 1 2 3 4 5 6 disp;			# displacements of end node
#recorder Node -file $dataDir/DB2Mid.out -time -node $MiddleNode -dof 1 2 3 4 5 6 disp;			# displacements of middle node
recorder Element -file $dataDir/MB1DefoEle1sec1.out -time -ele 1 section 1 deformation;
recorder Element -file $dataDir/MB1DefoEle1sec2.out -time -ele 1 section 2 deformation;
recorder Element -file $dataDir/MB1DefoEle1sec3.out -time -ele 1 section 3 deformation;
recorder Element -file $dataDir/MB1DefoEle1sec4.out -time -ele 1 section 4 deformation;
recorder Element -file $dataDir/MB1DefoEle1sec5.out -time -ele 1 section 5 deformation;
recorder Element -file $dataDir/MB1DefoEle1sec6.out -time -ele 1 section 6 deformation;
recorder Element -file $dataDir/MB1DefoEle1sec7.out -time -ele 1 section 7 deformation;

recorder Element -file $dataDir/MB1DefoEle1fromForce.out -time -ele 1 sectionDeformation_Force;
