...
set shearCoord [CsectionLipTurnUp $BeamSecTagFiber $IDsteel $D $B $L $t $r $nfdw $nfbf $nfL $nfC $nft];
...
# define RECORDERS
#-------------------------------------------------------------
recorder Node -file $dir/AsyCB_Nx300dispNomomeNoimper.out -time -node $middleNode $startNode $endNode -dof 1 2 3 4 5 6 7 disp;# record displacements at the middle node
recorder Node -file $dir/AsyCB_Nx300reacNomomeNoimper.out -time -node $startNode $endNode -dof 1 2 3 4 5 6 7 reaction;
# define second stage main Load (Axial force at the two ends)
#------------------------------------------------------------- 
set N 1.0;
pattern Plain 2 Linear {
  # NodeID, Fx, Fy, Fz, Mx, My, Mz, Bx
  load $startNode $N 0 0 0 [expr -$N*$z0] [expr -$N*$y0] 0; #when I eliminate moments, it can not converge
...
#set Dmax 10
set IDctrlNode $startNode; #Here use disp. control instead of rotation. I don't know why it doesn't converge
set IDctrlDOF 1; #if we dn't use rotation control, the resules show that rotation close to 0.
