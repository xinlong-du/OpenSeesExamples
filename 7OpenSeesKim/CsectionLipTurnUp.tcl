proc CsectionLipTurnUp {secID matID D B L t r nfdw nfbf nfL nfC nft} {
# ###################################################################
#Csection $BeamSecTagFiber $IDsteel $D $B $L $t $r $zbar $nfdw $nfbf $nfL $nfC $nft
####################################################################
# The Program creates the fiber section for lipped C section with round corners and one of the lips turned upwards
#and assign all relevant information required for fiber section
# ---------------------------------------------------------------------------------------------- 
# Written by: Rinchen  
# Date: 15 Dec 2015
...
#-------------------------------------------------------------
set data [roundCornerCSectionInfoLipTurnUp $D $B $L $t $r];#prepares data for calculation of section properties 
...
#and fiber section is based on actual section having finite thickness
	...
	set zc [expr 4/(3*$pi)*($R*$R*$R-$r*$r*$r)/($R*$R-$r*$r)]; #centroid of segment of annular ring
	set Areatrue [expr ($D-2*$R)*$t+2*($B-2*$R)*$t+2*($L-$R)*$t+$pi*($R*$R-$r*$r)]; #Area of section
	set Qcx [expr ($D-2*$R)*$t*$t/2+2*($B-2*$R)*$t*$B/2+2*($L-$R)*$t*($B-$t/2)+2*$pi*($R*$R-$r*$r)/4*($R-$zc)+2*$pi*($R*$R-$r*$r)/4*($B-$R+$zc)];
	set Qcy [expr ($D-2*$R)*$t*$D/2+($B-2*$R)*$t*($D-$t/2)+($B-2*$R)*$t*$t/2+($L-$R)*$t*($D+$r+($L-$R)/2)+($L-$R)*$t*($R+($L-$R)/2)+2*$pi*($R*$R-$r*$r)/4*($R-$zc)+$pi*($R*$R-$r*$r)/4*(2*$D-$t)];
	set zbar [expr $Qcx/$Areatrue]; #z-coordinate of centroid from left 
	set ybar [expr $Qcy/$Areatrue]; #y-coordinate of centroid from bottom left corner
#------------------------------------------------------------------	
#Create fiber cross-section information	in the rectangular axes
#-----------------------------------------------------------------
#Coordinates for comptuting fiber size and centroid
	set y1 [expr $D-$t+$L-$ybar]
	set y2 [expr $D-$t+$R-$ybar]
	set y3 [expr $y2-$r]
	set y4 [expr $y3-$t]
	set y5 [expr $y4-$r]
	set y6 [expr $L-$ybar]
...
#Calculate length of each fiber at each region: lips, flange, corners and web
	set dfL [expr ($y1-$y2)/($nfL)] ;#length of each fibre in lip
	set theta [expr ($pi/2)/$nfC]; #angle(radians) subtended by each fiber at the corner
	set cArea [expr ($R*$R-$r*$r)*$theta/2];#area of each corner fiber
	set dbf [expr ($z4-$z3)/$nfbf]; #length of each fiber in flange
	set dwf [expr ($y5-$y7)/$nfdw];#length of each fiber in web
	
	set NoOfFiber [expr ($nfL+$nfbf)*2+$nfC*4+$nfdw]; #Number of fibers

#Assemble coordinate of centroid, area of each fiber
#--------------------------------------------------------------------------------------------
 #create matrix to store fiber information
 set Fib [math::linearalgebra::mkMatrix $NoOfFiber 4 0];# fiberNo, z-coord, y-coord, Area
 
for {set i 0} {$i<$nfL} {incr i 1} {#Top lip
	#Fiber No, x,y, Area
	math::linearalgebra::setrow Fib $i $i 0 0;
	math::linearalgebra::setrow Fib $i [expr ($z5+$z6)/2] 1 1;
	math::linearalgebra::setrow Fib $i [expr $y1-($dfL/2*(2*$i+1))] 2 2;
...
for {set i 0} {$i<$nfC} {incr i 1} {#Top right corner
	...
	math::linearalgebra::setrow Fib $fibNo [expr $y2-($r+$t/2)*sin($theta/2*(2*$i+1))] 2 2;
...
for {set i 0} {$i<$nfbf} {incr i 1} {#Top flange
	...
	math::linearalgebra::setrow Fib $fibNo [expr ($y3+$y4)/2] 2 2;
...
for {set i 0} {$i<$nfC} {incr i 1} {#Top Left corner
	...
	math::linearalgebra::setrow Fib $fibNo [expr $y5+($r+$t/2)*sin($pi/2+$theta/2*(2*$i+1))] 2 2;
	math::linearalgebra::setrow Fib $fibNo $cArea 3 3;
	}
for {set i 0} {$i<$nfdw} {incr i 1} {#Web
	...
	math::linearalgebra::setrow Fib $fibNo [expr $y5-($dwf/2*(2*$i+1))] 2 2;
...
for {set i 0} {$i<$nfC} {incr i 1} {#Bottom Left corner
	...
	math::linearalgebra::setrow Fib $fibNo [expr $y7+($r+$t/2)*sin($pi+$theta/2*(2*$i+1))] 2 2;
	math::linearalgebra::setrow Fib $fibNo $cArea 3 3;
	}
for {set i 0} {$i<$nfbf} {incr i 1} {#Bottom flange
	...
	math::linearalgebra::setrow Fib $fibNo [expr -($ybar-$t/2)] 2 2;
	math::linearalgebra::setrow Fib $fibNo [expr $dbf*$t] 3 3;
	}
for {set i 0} {$i<$nfC} {incr i 1} {#Bottom right corner
	...
	math::linearalgebra::setrow Fib $fibNo [expr $y7+($r+$t/2)*sin(3*$pi/2+$theta/2*(2*$i+1))] 2 2;
	math::linearalgebra::setrow Fib $fibNo $cArea 3 3;
	}	
for {set i 0} {$i<$nfL} {incr i 1} {#bottom lip
	...
	math::linearalgebra::setrow Fib $fibNo [expr $y7+($dfL/2*(2*$i+1))] 2 2;
	math::linearalgebra::setrow Fib $fibNo [expr $dfL*$t] 3 3;
	}
#-----------------------------------------------------------------------------------------------------------------
#Convert centroidal coordinates of fibers from rectangular axes to principal axes
set FibPAxis [math::linearalgebra::mkMatrix $NoOfFiber 3 0];# stores fiber coordinates in principal axes
for {set i 0} {$i<$NoOfFiber} {incr i 1} {
	set fId($i) [math::linearalgebra::getelem $Fib $i 0];# fiber number
	set f_z($i) [math::linearalgebra::getelem $Fib $i 1];# z-coordinate in rectangular axes
	set f_y($i) [math::linearalgebra::getelem $Fib $i 2];# y-coordinate in rectangular axes
	set f_A($i) [math::linearalgebra::getelem $Fib $i 3];# fiber area
	
	#Convert the coordinates to principal coordinates
	set fpz($i) [expr $f_y($i)*sin($alpha)+$f_z($i)*cos($alpha)];#z-coordinates in principal axes
	set fpy($i) [expr $f_y($i)*cos($alpha)-$f_z($i)*sin($alpha)];#y-coordinates in principal axes
	
	#Assemble fiber coordinates in principal axes and stores in $FibPAxis
	math::linearalgebra::setrow FibPAxis $i $i 0 0;
	math::linearalgebra::setrow FibPAxis $i $fpz($i) 1 1;
	math::linearalgebra::setrow FibPAxis $i $fpy($i) 2 2;
}

#Interpolate and assign sectorial coordinates to fiber
#---------------------------------------------------
set secCoord [Interpolate $FibPAxis $corwarpInfo];# calls interpolation functions to calculate sectorial coordinates 
for {set i 0} {$i<$NoOfFiber} {incr i 1} {
	set wn($i) [lindex $secCoord $i 1]; #extracts sectorial coordinates and stores in $wn
}

#Create section fiber
section Fiber $secID { 
	for {set i 0} {$i<$NoOfFiber} {incr i 1} {
	fiber $fpy($i) $fpz($i) $f_A($i) $matID $wn($i) $yo $zo; #yLoc, zLoc, Area, materialID, sectorial coordinate, y_coord, and z_coord of shear centre
	}
	}
return $shearCentre
}
