...
#	PI-Section
#------------------------------------------------------------------------------------------------------
source C:/Tcl/lib/tcllib1.17/math/math.tcl; # address to folder where math library is stored
...
#This procedure returns the node coordinate and element information matrix for C section with round corners  
#-------------------------------------------------------------------------------------------------------------
proc roundCornerTroughInfo {D B L t r} {
#
#define coordinates
#
#              (Node1x,Node1y)
...
#       11--------------(11)-------------12
#              
#  Cross-section of Trough-section 
#
set NoOfNodes 22; # input number of nodes
set NoOfElements [expr $NoOfNodes-1];
...
#  Define centraline coordinates of nodes with origin at bottom left corner
	...
	#Top Left lip
	set Node1x [expr ($L-0.5*$t)]; 	set Node1y [expr $B-$t];	
	set Node2x [expr $rm]; 		set Node2y [expr $B-$t];
			
	#Top left corner
	set Node3x [expr $Node2x-$rm*sin($theta)]; 	set Node3y [expr $Node2y-$rm+$rm*cos($theta)];	
	set Node4x [expr $Node2x-$rm*sin(2*$theta)]; 	set Node4y [expr $Node2y-$rm+$rm*cos(2*$theta)];	
	set Node5x [expr $Node2x-$rm*sin(3*$theta)]; 	set Node5y [expr $Node2y-$rm+$rm*cos(3*$theta)];
		
	#Left flange
	set Node6x 0.0; 	set Node6y [expr $B-$t-$rm]; 	
	set Node7x 0.0; 	set Node7y $rm; 
	
	#Bottom left corner
	set Node8x $Node5x; 	set Node8y [expr $rm-$rm*sin($theta)]; 	
	set Node9x $Node4x; 	set Node9y [expr $rm-$rm*sin(2*$theta)]; 	
	set Node10x $Node3x; 	set Node10y [expr $rm-$rm*sin(3*$theta)]; 
	
	#Web
	set Node11x $Node2x; 		set Node11y 0.0; 	
	set Node12x [expr $D-$t-$rm]; 	set Node12y 0.0; 
	
	##Bottom right corner
	set Node13x [expr $Node12x+$rm*sin($theta)]; 	set Node13y $Node10y; 	
	set Node14x [expr $Node12x+$rm*sin(2*$theta)]; 	set Node14y $Node9y; 	
	set Node15x [expr $Node12x+$rm*sin(3*$theta)]; 	set Node15y $Node8y; 
		
	#Right flange
	set Node16x [expr $D-$t]; 	set Node16y $Node7y; 	
	set Node17x $Node16x; 		set Node17y $Node6y; 
	
	#Top right corner
	set Node18x $Node15x; 	set Node18y $Node5y; 	
	set Node19x $Node14x; 	set Node19y $Node4y; 	
	set Node20x $Node13x; 	set Node20y $Node3y; 
		
	#Top right lip
	set Node21x $Node12x; 			set Node21y $Node2y;	
	set Node22x [expr $Node16x-$Node1x]; 	set Node22y $Node1y; 
#---------------------------------------------------------------------------	
#construct nodal coordinate matrix based on the cross-section information
set NodeCoord [math::linearalgebra::mkMatrix $NoOfNodes 3 0.0]
for {set i 0} {$i<$NoOfNodes} {incr i 1} {
...
#
#====================================================================================================================================
#This procedure returns the node coordinate and element information matrix for C section with round corners and lips turn upside down  
#---------------------------------------------------------------------------------------------------------------------------
proc roundCornerCSectionInfoLipTurn {D B L t r} {
#
#define coordinates
#               
#         7-----(6)-----6
...
#			22(Node22x,Node22y)
#
#  Cross-section of C-section with lips turn downwards
#
set NoOfNodes 22; # input number of nodes
set NoOfElements [expr $NoOfNodes-1];
set nfC 4; #4 segments at the corner
...
#  Define centraline coordinates of nodes with origin at bottom left corner
	...
	#Top lip
	set Node1x [expr $B-$t]; 	set Node1y [expr $D-$t];	
	set Node2x [expr $B-$t]; 	set Node2y [expr $D-$t+$L-0.5*$t-$rm];
	...
	#Top flange
	set Node6x [expr $B-$t-$rm]; 	set Node6y [expr $D-$t+$L-0.5*$t];
	set Node7x $rm; 		set Node7y $Node6y; 
	...
	#Web
	set Node11x 0.0; 	set Node11y $Node2y; 
	set Node12x 0.0; 	set Node12y [expr $L-0.5*$t+$rm]; 
	
	#Bottom left corner
	set Node13x [expr $rm-$rm*cos($theta)]; 	set Node13y [expr $Node12y-$rm*sin($theta)]; 
	set Node14x [expr $rm-$rm*cos(2*$theta)]; 	set Node14y [expr $Node12y-$rm*sin(2*$theta)]; 
	set Node15x [expr $rm-$rm*cos(3*$theta)]; 	set Node15y [expr $Node12y-$rm*sin(3*$theta)];

	#Bottom flange
	set Node16x $Node7x; 	set Node16y [expr $L-0.5*$t];
	set Node17x $Node6x; 	set Node17y $Node16y;
	
	#Bottom right corner
	set Node18x $Node5x; 	set Node18y [expr ($Node17y-$rm+$rm*cos($theta))]; 
	set Node19x $Node4x; 	set Node19y [expr ($Node17y-$rm+$rm*cos(2*$theta))]; 
	set Node20x $Node3x; 	set Node20y [expr ($Node17y-$rm+$rm*cos(3*$theta))];
	
	#Bottom lip
	set Node21x $Node2x; 	set Node21y [expr $L-0.5*$t-$rm]; 
	set Node22x $Node1x; 	set Node22y 0.0; 
#---------------------------------------------------------------------------	
#construct nodal coordinate matrix based on the cross-section information
set NodeCoord [math::linearalgebra::mkMatrix $NoOfNodes 3 0.0]
for {set i 0} {$i<$NoOfNodes} {incr i 1} {
...
#  Define centraline coordinates of nodes with origin at bottom left corner
	...
	#Top lip
	set Node1x [expr $B-$t]; 	set Node1y [expr $D-$t+$L-$t/2];
	set Node2x [expr $B-$t]; 	set Node2y [expr $D-$t+$rm];
	...
	#Top left corner
	set Node8x [expr $rm-$rm*sin($theta)]; 		set Node8y [expr $Node7y-$rm+$rm*cos($theta)];
	set Node9x [expr $rm-$rm*sin(2*$theta)]; 	set Node9y [expr $Node7y-$rm+$rm*cos(2*$theta)];
	set Node10x [expr $rm-$rm*sin(3*$theta)]; 	set Node10y [expr $Node7y-$rm+$rm*cos(3*$theta)];
	
	#Web
	set Node11x 0.0; 	set Node11y [expr $D-$t-$rm];
	set Node12x 0.0; 	set Node12y $rm;
	...
	#Bottom right corner
	set Node18x $Node5x; 	set Node18y [expr $rm-$rm*cos($theta)];
	set Node19x $Node4x; 	set Node19y [expr $rm-$rm*cos(2*$theta)];
	set Node20x $Node3x; 	set Node20y [expr $rm-$rm*cos(3*$theta)];
	
	#Bottom lip
	set Node21x $Node2x; 	set Node21y $rm;
	set Node22x $Node1x; 	set Node22y [expr $L-$t/2];
#---------------------------------------------------------------------------	
#construct nodal coordinate matrix based on the cross-section information
set NodeCoord [math::linearalgebra::mkMatrix $NoOfNodes 3 0.0]
for {set i 0} {$i<$NoOfNodes} {incr i 1} {
...
#================================================================================================================
#This procedure returns the node coordinate and element information matrix for PI section
#----------------------------------------------------------------------------------------------------------------

proc piSectionInfo {D B Bw tf tw} {
#D: Depth
#B: Flange width
#Bw: clear witdth between legs
#tf/tw thickness of web/flange
#define coordinates
#                
#     1----(1)----2-----(2)---3-----(3)-----4
#    		  |           |
...
#		  5           6
#              
#  Cross-section of PI-section             
#
set NoOfNodes 6; 
set NoOfElements [expr $NoOfNodes-1]

#  Define centraline coordinates of nodes with origin at bottom left 
	set Node1x 0.0; 			set Node1y [expr ($D-0.5*$tf)]; 	
	set Node2x [expr ($B-($Bw+$tw))/2]; 	set Node2y $Node1y;	
	set Node3x [expr ($Node2x+$Bw+$tw)]; 	set Node3y $Node1y; 	
	set Node4x $B; 				set Node4y $Node1y; 	
	set Node5x $Node2x; 			set Node5y 0.0; 	
	set Node6x $Node3x; 			set Node6y 0.0; 
#---------------------------------------------------------------------------
#construct nodal coordinate matrix based on the cross-section information
set NodeCoord [math::linearalgebra::mkMatrix $NoOfNodes 3 0.0]; #No of rows, No of columns default value
for {set i 0} {$i<$NoOfNodes} {incr i 1} {
...
#Define element connection (startnode i) in column1: colNo,startNodei, min, max
math::linearalgebra::setcol Segment 1 1 0 0;
math::linearalgebra::setcol Segment 1 2 1 1;
math::linearalgebra::setcol Segment 1 3 2 2;
math::linearalgebra::setcol Segment 1 2 3 3;
math::linearalgebra::setcol Segment 1 3 4 4;
#Define element connection (endnode j) in column 2:colNo, endNodej, min, max
math::linearalgebra::setcol Segment 2 2 0 0;
math::linearalgebra::setcol Segment 2 3 1 1;
math::linearalgebra::setcol Segment 2 4 2 2;
math::linearalgebra::setcol Segment 2 5 3 3;
...
math::linearalgebra::setcol Segment 3 $tf 1 1;
math::linearalgebra::setcol Segment 3 $tf 2 2;
math::linearalgebra::setcol Segment 3 $tw 3 3;
math::linearalgebra::setcol Segment 3 $tw 4 4;
#---------------------------------------------
return [list $NodeCoord $Segment]
}