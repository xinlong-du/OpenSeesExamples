...
for {set i 1} {$i<12} {incr i 1} {
	node $i 0.0 [expr -120.0+120.0*$i] 0.0;
}
for {set i 12} {$i<22} {incr i 1} {
	node $i [expr 120.0*($i-11)] 1200.0 0.0;
}
...
# Define RECORDERS -------------------------------------------------------------
recorder Node -file $dataDir/LeeFrameElasDB20.out -time -node $MiddleNode -dof 1 2 3 4 5 6 disp;			# displacements of middle node
...
integrator ArcLength 8.0 2.0; #arclength alpha
#set Dincr -0.5; #-0.00002
                               #Node,     dof, 1st incr, Jd, min,   max
#integrator DisplacementControl $MiddleNode 2   $Dincr     1  $Dincr -0.5;
...
algorithm Newton;
#integrator LoadControl 0.1;
#integrator ArcLength 2.0 0.1; #arclength alpha
