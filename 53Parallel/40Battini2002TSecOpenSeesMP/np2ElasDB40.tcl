...
if {$pid == 0} {
	...
	#Nodes, NodeNumber, xCoord, yCoord, zCoord
    for {set i 21} {$i<42} {incr i 1} {
		node $i [expr -45.0+45.0*$i] 0 0;
    }
    fix 41  0 1 1 1 1 1;
    for {set i 21} {$i<41} {incr i 1} {
		set elemID $i
		set nodeI $i
		set nodeJ [expr $i+1]
		element dispBeamColumn $elemID $nodeI $nodeJ $numIntgrPts $ColSecTag $IDColTransf $y0 $z0;	
...
# Define RECORDERS -------------------------------------------------------------
recorder Node -file $dataDir/np2ElasDispEndDB40.out.$pid -time -node 41 -dof 1 2 3 4 5 6 disp;			# displacements of end node
recorder Node -file $dataDir/np2ElasDispMidDB40.out.$pid -time -node 21 -dof 1 2 3 4 5 6 disp;			# displacements of middle node
