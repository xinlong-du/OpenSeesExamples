...
set np [getNP]
if {$np != 8} {
    puts "ONLY WORKS FOR 8 PROCESSORS"
    exit
}
...
if {$pid == 0} {
	...
	#Nodes, NodeNumber, xCoord, yCoord, zCoord
    for {set i 11} {$i<17} {incr i 1} {
		node $i [expr -45.0+45.0*$i] 0 0;
    }
    for {set i 11} {$i<16} {incr i 1} {
		set elemID $i
		set nodeI $i
		set nodeJ [expr $i+1]
		element dispBeamColumn $elemID $nodeI $nodeJ $numIntgrPts $ColSecTag $IDColTransf $y0 $z0;	
	...
	#Nodes, NodeNumber, xCoord, yCoord, zCoord
    for {set i 16} {$i<22} {incr i 1} {
		node $i [expr -45.0+45.0*$i] 0 0;
    }
    for {set i 16} {$i<21} {incr i 1} {
		set elemID $i
		set nodeI $i
		set nodeJ [expr $i+1]
		element dispBeamColumn $elemID $nodeI $nodeJ $numIntgrPts $ColSecTag $IDColTransf $y0 $z0;	
	...
	#Nodes, NodeNumber, xCoord, yCoord, zCoord
    for {set i 21} {$i<27} {incr i 1} {
		node $i [expr -45.0+45.0*$i] 0 0;
    }
    for {set i 21} {$i<26} {incr i 1} {
		set elemID $i
		set nodeI $i
		set nodeJ [expr $i+1]
		element dispBeamColumn $elemID $nodeI $nodeJ $numIntgrPts $ColSecTag $IDColTransf $y0 $z0;	
	...
	#Nodes, NodeNumber, xCoord, yCoord, zCoord
    for {set i 26} {$i<32} {incr i 1} {
		node $i [expr -45.0+45.0*$i] 0 0;
    }
    for {set i 26} {$i<31} {incr i 1} {
		set elemID $i
		set nodeI $i
		set nodeJ [expr $i+1]
		element dispBeamColumn $elemID $nodeI $nodeJ $numIntgrPts $ColSecTag $IDColTransf $y0 $z0;	
	...
	#Nodes, NodeNumber, xCoord, yCoord, zCoord
    for {set i 31} {$i<37} {incr i 1} {
		node $i [expr -45.0+45.0*$i] 0 0;
    }
    for {set i 31} {$i<36} {incr i 1} {
		set elemID $i
		set nodeI $i
		set nodeJ [expr $i+1]
		element dispBeamColumn $elemID $nodeI $nodeJ $numIntgrPts $ColSecTag $IDColTransf $y0 $z0;	
	...
	#Nodes, NodeNumber, xCoord, yCoord, zCoord
    for {set i 36} {$i<42} {incr i 1} {
		node $i [expr -45.0+45.0*$i] 0 0;
    }
    fix 41  0 1 1 1 1 1;
    for {set i 36} {$i<41} {incr i 1} {
		set elemID $i
		set nodeI $i
		set nodeJ [expr $i+1]
		element dispBeamColumn $elemID $nodeI $nodeJ $numIntgrPts $ColSecTag $IDColTransf $y0 $z0;	
...
# Define RECORDERS -------------------------------------------------------------
recorder Node -file $dataDir/np8ElasDispEndDB40.out.$pid -time -node 41 -dof 1 2 3 4 5 6 disp;			# displacements of end node
recorder Node -file $dataDir/np8ElasDispMidDB40.out.$pid -time -node 21 -dof 1 2 3 4 5 6 disp;			# displacements of middle node
