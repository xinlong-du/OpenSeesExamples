...
if {$pid == 0} {
# define 1ST ANALYSIS----------------------------------------------------------------
recorder Node -file $dataDir/fact250DB40stp1.out.$pid -time -node $EndCentroid -dof 1 2 3 4 5 6 disp;
...
# define 2ND ANALYSIS----------------------------------------------------------------
recorder Node -file $dataDir/fact250DB40stp2.out.$pid -time -node $EndCentroid -dof 1 2 3 4 5 6 disp;
DisplayModel3D DeformedShape;	 # options: DeformedShape NodeNumbers ModeShape
...
#analyze 34300000 0.0000001;					# Central Difference
analyze 1000 0.001;
puts "Done 2nd analysis of pid 0"
} elseif {$pid == 1} {
# define 1ST ANALYSIS----------------------------------------------------------------
# Define RECORDERS ------------------------------------------------------------------
recorder Node -file $dataDir/fact1000DB40stp1.out.$pid -time -node $EndCentroid -dof 1 2 3 4 5 6 disp;
DisplayModel3D DeformedShape;	 # options: DeformedShape NodeNumbers ModeShape

pattern Plain 4 $WindSeriesA -fact 1000.0 {
	load $EndLoad  0.0 -50.0 25.0 0.0 0.0 0.0;  
}
...
#analyze 34300000 0.0000001;					# Central Difference
analyze 1000 0.001;
puts "Done 1st analysis of pid 1"

# CALL DATABASE----------------------------------------------------------------------------
set dbDir db;			      # set up name of data directory
file mkdir $dbDir; 			  # create data directory

database File db/DB40pid1;
save 72;
...
# define 2ND ANALYSIS----------------------------------------------------------------
recorder Node -file $dataDir/fact1000DB40stp2.out.$pid -time -node $EndCentroid -dof 1 2 3 4 5 6 disp;
DisplayModel3D DeformedShape;	 # options: DeformedShape NodeNumbers ModeShape
...
#analyze 34300000 0.0000001;					# Central Difference
analyze 1000 0.001;
puts "Done 2nd analysis of pid 1"
}
